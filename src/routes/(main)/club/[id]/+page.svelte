<script lang="ts">
    ...
    $: zooProject = data.zooProject;
  
    const animatedTitleIndex = tweened(0, { duration: 600 });
    $: animatedTitleOutput =
     zooProject.title.slice(0, $animatedTitleIndex) +
      ($animatedTitleIndex === zooProject.title.length ? '' : '|');
  
    async function resetAnimation() {
      await animatedTitleIndex.set(0, { duration: 0 });
      animatedTitleIndex.set(zooProject.title.length);
    }
  
    $: zooProject.title, resetAnimation();
  </script>
    ...
    <div class="flex-[4] min-h-screen">
      <div class="flex flex-col pt-6 md:pt-8">
        ...
        <div class="flex-1 mt-6">
          <div class="md:text-lg mb-6">{zooProject.overview}</div>
  
          <ul class="flex flex-col gap-6">
            {#each zooProject.bullets as bullet}
      ...
      >
        <p class="font-semibold font-sans">Franklin Software Developers</p>
